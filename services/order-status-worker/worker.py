...
QUEUE_NAME = os.getenv("DECISION_QUEUE_NAME", "order-decision-queue")
DATABASE_URL = os.getenv("DATABASE_URL", "postgresql://app:app@postgres:5432/orders")
HANDLER_NAME = "order-status-worker"
    ...
    with db_conn() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                """
                UPDATE orders
                SET status = %s, last_event_id = %s, updated_at = NOW()
                WHERE order_id = %s
                """,
                (new_status, event["eventId"], event["orderId"]),
            )
            cur.execute(
...
def run():
    ...
    for attempt in range(30):
        try:
            ...
            print(f"[order-status-worker] queue not ready ({exc}), retry={attempt + 1}")
            time.sleep(2)

    if not queue_url:
        raise RuntimeError("could not resolve decision queue url")

    print(f"[order-status-worker] listening on {QUEUE_NAME} ({queue_url})")
    ...
    while True:
        ...
        for msg in resp.get("Messages", []):
            ...
            try:
                ...
                if result != "IGNORED":
                    print(
                        "[order-status-worker] updated "
                        f"orderId={event.get('orderId')} eventType={event.get('eventType')} result={result}"
                    )
                sqs.delete_message(QueueUrl=queue_url, ReceiptHandle=receipt_handle)
            except Exception as exc:
                print(f"[order-status-worker] failed to process message: {exc}")
